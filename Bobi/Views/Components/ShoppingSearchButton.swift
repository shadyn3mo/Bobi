...
struct ShoppingSearchButton: View {
    ...
    var body: some View {
            ...
            if isExpanded {
                ...
                Button(action: expandSearch) {
                    HStack(spacing: ResponsiveDesign.Spacing.small) {
                        ...
                        Text("shopping.search.title".localized)
                            .foregroundColor(secondaryTextColor)
                            .font(.system(size: 14, weight: .medium))
                    }
        ...
        .frame(minWidth: isExpanded ? nil : ResponsiveDesign.ButtonSize.extraLarge * 2.2)
...
#Preview {
    ...
    return ZStack {
        ...
        VStack(spacing: 30) {
            // 正常状态
            ShoppingSearchButton(searchText: $searchText, isExpanded: $isExpanded)
            
            // 手动展开状态演示
            ShoppingSearchButton(searchText: .constant("示例搜索"), isExpanded: .constant(true))
        }
        .padding()
    }
}